...
File:        model_calibration_USA_endogenous_open.py
Purpose:     Calibrate the open-economy model on US data under ENDOGENOUS trade:
             sectoral exports are generated by the model through trade elasticities
             xi_i (export-intensity parameters) that load on sectoral
             productivity. Recovers the xi_i's together with US sectoral
             productivities so that the model reproduces observed US net-export
             and employment-share paths.
Pipeline:    Step 7/19 — Endogenous-trade calibration on US.
Inputs:      ../data/euklems_2023.csv (EUKLEMS 2023 VA_Q, H by sector)
             ../data/raw/OECD_GDP_ph.xlsx (OECD GDP per hour, PPP USD)
             ../data/io_panel.xlsx (sectoral exports and imports, OECD ICIO)
             ../data/exp_imp_aggregate_panel.xlsx (aggregate trade totals)
Outputs:     Module-level variables: US productivities A_*, E, GDP_ph, sectoral
             shares share_* and their "_closed" counterparts, trade elasticities
             xi_* (xi_agr, xi_man, xi_trd, xi_bss, xi_fin, xi_nps), and the
             "year" index used by downstream scripts to align European data.
Dependencies: model_calibration_USA.py (imports sigma and eps_* preferences).
"""
...
# Construct a sectoral imports-as-a-share-of-GDP INDEX: set the initial level
# to the exports-to-GDP ratio (a normalization that anchors the model in
# balanced trade in the first period, consistent with the paper's Section 6
# initial-period calibration) and then propagate using the observed growth
# rate of imports/GDP. The resulting net-export series nx_i = x_i - m_i_index
# is the object fed into the sectoral resource constraint.
...
def xi_fit_sector(x_q_index, A):
    """Calibrate the sectoral trade elasticity xi_i of the endogenous-export
    rule x_i,t = x_0,i * A_i,t^xi_i (equivalently, x_i = x_0,i * p_i^{-xi_i}
    since p_i = 1/A_i under linear-in-labor production; see Section 6 of the
    paper). xi_i is identified by requiring the model-implied terminal-period
    export quantity to match the observed value, holding the initial level
    fixed. The objective is (model_last - data_last)^2; the outer fit is
    exactly identified (one parameter, one moment) so the minimum is zero."""
...
"Non-homothetic CES index (endogenous-trade form). Recovers C from the"
"relative-labor equation purged of the trade wedge: (l_i - nx_i) is domestic"
"absorption in sector i, so (l_i - nx_i)/(l_m - nx_m) is the relative"
"domestic-consumption allocation implied by the Comin et al. (2021) CES."
...
class model_ams:
    """Three-sector open-economy model (agr, man, ser) with ENDOGENOUS sectoral exports x_i = x_0,i * A_i^xi_i (equivalently p_i^{-xi_i}) and exogenous import ratios M_i_E; consistent with Section 6 of the paper. Net exports are now a function of sectoral productivity and thus respond to productivity shocks in the counterfactuals."""
    ...
    def labor_agr(self, C, E, A_agr, M_agr_E):
        """Sectoral labor share in agriculture. Equals the closed-economy CES weight plus the trade wedge (E^(1-sigma))*(x_agr - M_agr_E), where endogenous exports x_agr = x_0,agr * A_agr^xi_agr / E respond to productivity via xi_agr. Imports M_agr_E enter as an exogenous path anchored in the initial period."""
...
class model_nps:
    """Six-sector open-economy endogenous-trade model (agr, man, trd, bss, fin, nps). Same endogenous-export rule x_i = x_0,i * A_i^xi_i as model_ams, with services disaggregated; this is the specification used in Table 6 and Figure 6 of the paper."""
